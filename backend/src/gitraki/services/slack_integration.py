...
from gitraki.models.integrations import SlackIntegration, SlackNotificationLog
...
class SlackIntegrationService:
    ...
    async def handle_slash_command(
        ...
        if not integration or not integration.is_active:
            return SlackCommandResponse(
                text="Gitraki is not installed in this workspace. Please install it first.",
            )
    ...
    async def _handle_profile_command(
        ...
        """Handle /gitraki profile @user command."""
        ...
        if not developer:
            return SlackCommandResponse(
                text=f"Developer not found. Make sure the user exists in Gitraki.",
            )
    ...
    async def _handle_match_command(
        ...
        """Handle /gitraki match "task description" command."""
        if not args:
            return SlackCommandResponse(
                text='Please provide a task description. Example: `/gitraki match "Fix authentication bug in OAuth flow"`',
            )
    ...
    async def _handle_insights_command(
        ...
        """Handle /gitraki insights command."""
        return SlackCommandResponse(
            response_type="ephemeral",
            text=":crystal_ball: *Team Insights*\n\n_Use the Gitraki web dashboard for detailed predictive insights including attrition risk, burnout indicators, and performance trajectories._",
        )
    ...
    async def _handle_report_command(
        ...
        """Handle /gitraki report command."""
        return SlackCommandResponse(
            response_type="ephemeral",
            text=":bar_chart: *Reports*\n\n_Use the Gitraki web dashboard to create and schedule custom reports. Scheduled reports can be delivered to Slack channels automatically._",
        )
    ...
    async def _handle_help_command(
        ...
        """Handle /gitraki help command."""
        blocks = [
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
                    "text": ":wave: *Gitraki Commands*",
                },
            },
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
                    "text": """• `/gitraki profile [@user]` - View developer profile
• `/gitraki match "task"` - Find best developer for a task
• `/gitraki team` - Team skill overview
• `/gitraki insights` - Team health summary
• `/gitraki report` - Report information
• `/gitraki help` - Show this help message""",
                },
            },
            {
                "type": "context",
                "elements": [
                    {
                        "type": "mrkdwn",
                        "text": "For full features, visit the Gitraki web dashboard.",
                    }
                ],
            },
        ]
    ...
    async def _handle_app_mention(
        ...
        """Handle @gitraki mentions."""
        channel = event_data.get("channel")
        thread_ts = event_data.get("thread_ts") or event_data.get("ts")

        message = SlackMessage(
            text="Hi! Use `/gitraki help` to see available commands.",
            thread_ts=thread_ts,
        )
    ...
    async def _handle_direct_message(
        ...
        message = SlackMessage(
            text="Hi! I'm Gitraki. Use `/gitraki help` in any channel to see available commands.",
        )
